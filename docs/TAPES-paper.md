...
```mermaid
flowchart TD
    T["<b>T: Trigger</b><br/>Any inbound signal or event"]
    A["<b>A: Acquire Context</b><br/>Build understanding of the task"]
    P["<b>P: Plan</b><br/>Decompose, sequence, record intent"]
    E["<b>E: Execute</b><br/>Implement, verify, iterate to completion"]
    S["<b>S: Ship</b><br/>Publish output, notify, close the loop"]

    GA{{"Clarification Gate A<br/><i>needs more context</i>"}}
    GP{{"Clarification Gate P<br/><i>needs more context</i>"}}
    GE{{"Clarification Gate E<br/><i>needs more context</i>"}}

    T --> A
    A --> GA
    GA -- "resolved" --> P
    GA -. "returns to phase" .-> A
    P --> GP
    GP -- "resolved" --> E
    GP -. "returns to phase" .-> P
    E --> GE
    GE -- "resolved" --> S
    GE -. "returns to phase" .-> E
```
