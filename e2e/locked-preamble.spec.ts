...
// Seeded ACQUIRE agent config ID from prisma/seed.ts
const ACQUIRE_AGENT_CONFIG_ID = '00000000-0000-0000-0000-000000000102';
...
test.describe('Test Case 3: Locked Preamble Guard', () => {
  test('LockedPreambleGuard rejects prompts containing gate condition patterns', async ({ request }) => {
    ...
    // Try to patch the seeded agent with a gate condition pattern in systemPromptBody
    const res = await request.patch(`${API_BASE}/api/agents/${ACQUIRE_AGENT_CONFIG_ID}`, {
      data: {
  ...
  test('LockedPreambleGuard allows prompts without gate patterns', async ({ request }) => {
    ...
    // Patch seeded agent with a normal systemPromptBody (no gate patterns)
    const res = await request.patch(`${API_BASE}/api/agents/${ACQUIRE_AGENT_CONFIG_ID}`, {
      data: {
