...
describe('PlanAgentService', () => {
  ...
  let service: PlanAgentService;

  const makeContext = () => ({
    runId: 'r1', harnessId: 'h1', phase: 'PLAN' as const,
    agentConfig: {
      agentId: 'plan-default', position: 0, llmConnectorId: 'anthropic',
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new PlanAgentService(mockDispatcher as never);
  });
  ...
  it('buildInitialMessage includes context details', () => {
    const msg = service.buildInitialMessage({
      runId: 'r1', harnessId: 'h1', hasGap: false, files: ['a.ts', 'b.ts'], dependencies: ['dep1'],
    });
  ...
  it('runPlan returns PlanArtifact on success', async () => {
    mockLLM.complete.mockResolvedValue({
      text: JSON.stringify({ hasGap: false, steps: ['do stuff'] }),
      content: [], toolUses: [], usage: { inputTokens: 10, outputTokens: 5 }, stopReason: 'end_turn',
    });

    const input = { runId: 'r1', harnessId: 'h1', hasGap: false, files: [], dependencies: [] };
    const result = await service.runPlan(input, makeContext() as never);
  ...
  it('runPlan returns GateEvent when gate fired', async () => {
    mockLLM.complete.mockResolvedValue({
